...
describe("SimpleBondingCurve", () => {
  ...
  it("Should Buy", async () => {
    ...
    const addressbal = await bondingCurve.balanceOf(owner.address).then((d) => d.toNumber());
  ...
  it("Should Sell", async () => {
    ...
    const [owner] = await ethers.getSigners();
    const bal = await bondingCurve.balanceOf(owner.address).then((d) => d.toNumber());
    ...
    const balAfter = await bondingCurve.balanceOf(owner.address).then((d) => d.toNumber());
  ...
  it("Should Set Gas Price", async () => {
    ...
    const rec = await bondingCurve.setGasPrice(newGasPrice).then((res) => res.wait());
